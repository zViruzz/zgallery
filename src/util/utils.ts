import { type OrderParameter, SORT_TYPE } from '@/static/static'
import type { ExtendedFileType } from '@/type'

export async function getResolutionImage(
	file: File,
...
export async function getResolutionVideo(
	file: File,
): Promise<{ width: number; height: number }> {
	return await new Promise((resolve, reject) => {
		if (file !== null) {
...
export function getNameAndFormat(
	text: string,
): { nameBefore: string; format: string } | string {
	// const regex = /(.+)(\(\d+\))/
...
export async function getVideoThumbnailUrl(
	...
	return await new Promise((resolve, reject) => {
		if (file.type.match('video') !== null) {
			importFileandPreview(file, false).then((urlOfFIle) => {
				const video = document.createElement('video')
				const timeupdate = () => {
					if (snapImage()) {
...
export async function getVideoThumbnail(
	...
	return await new Promise((resolve, reject) => {
		getVideoThumbnailUrl(file, videoTimeInSeconds).then((url) => {
			fetch(url)
...
export function sortList(list: ExtendedFileType[], order: OrderParameter): any[] {
	...
	if (order === SORT_TYPE.RECENT_INVERT) {
		return list.reverse()
	}
	if (order === SORT_TYPE.A_Z) {
